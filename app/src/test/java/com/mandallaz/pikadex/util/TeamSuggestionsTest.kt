...
class TeamSuggestionsTest {
    ...
    @Test
    fun `computeOffensiveMultipliers is precomputed only once per type`() {
        var offensiveAccessCount = 0
        val countingList = CountingList(listOf(ref("ground"))) {
            offensiveAccessCount++
        }
        val customGrass = TypeDetailDto(
            id = 0,
            name = "grass",
            damageRelations = DamageRelations(
                doubleDamageFrom = emptyList(),
                doubleDamageTo = countingList,
        ...
        val customDetails = mapOf("grass" to customGrass, "ground" to ground)
        val candidates = listOf(
            SuggestionCandidate("victreebel", listOf("grass"), statTotal = 490),
            SuggestionCandidate("vileplume", listOf("grass"), statTotal = 490)
        )

        val result = rankSuggestions(
            sharedWeaknesses = emptyList(),
            coverageGaps = listOf("ground"),
            candidates = candidates,
            typeDetailsByType = customDetails,
            excludeNames = emptySet()
        )

        assertEquals(2, result.size)
        // With precomputation once per type, customGrass's doubleDamageTo is accessed exactly once.
        // If it were computed per candidate, it would be accessed twice (once for victreebel and once for vileplume).
        assertEquals(1, offensiveAccessCount)
    }

    @Test
    fun `computeDefensiveMultipliers is memoized per type-pair key`() {
        var defensiveAccessCount = 0
        val countingList = CountingList(listOf(ref("water"))) {
            defensiveAccessCount++
        }
        val customGrass = TypeDetailDto(
            id = 0,
            name = "grass",
        ...
        val customDetails = mapOf("grass" to customGrass, "water" to water)
        val candidates = listOf(
            SuggestionCandidate("victreebel", listOf("grass"), statTotal = 490),
            SuggestionCandidate("vileplume", listOf("grass"), statTotal = 490)
        )
        ...
        assertEquals(2, result.size)
        // With memoization, customGrass's halfDamageFrom is accessed exactly once for the type-pair ["grass"].
        // If it were computed per candidate, it would be accessed twice (once for victreebel and once for vileplume).
        assertEquals(1, defensiveAccessCount)
    }
}

private class CountingList<T>(private val delegate: List<T>, val onAccess: () -> Unit) : List<T> by delegate {
