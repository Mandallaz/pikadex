 ...
 *  tips at least half the roster. [immunitiesGained]/[quadWeaknessesGained] are the per-member
 *  counterpart added after user feedback that adding e.g. Toedscool (Ground/Grass) to an
 *  all-Fire preset team didn't mention it bringing the team's first immunity to Electric, or its
 *  own ×4 weakness to Ice — real, single-Pokémon-driven changes the majority rule is blind to. */
 ...
 *  gained/closed direction for the immunity and quad-weakness axes: the caller only ever *adds* a
 *  candidate (never removes one), and a team's collective min (immunity) and max (quad weakness)
 *  multiplier per type can only move toward "more covered" as members are added, never regress —
 *  so there's no symmetric "lost" case to compute here. */
