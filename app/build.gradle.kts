...
dependencies {
    ...
    // F112 — installs the baseline profile (generated by :baselineprofile, see #175) into
    // ART on first run, ahead of Play Store's own profile delivery kicking in.
    implementation(libs.androidx.profileinstaller)

    // F112 — the special `baselineProfile` configuration is how the androidx.baselineprofile
    // plugin locates the module that generates the profile (#174/#175's :baselineprofile), rather
    // than a `baselineProfile { }` build-script DSL block on this (the consuming) module's side.
